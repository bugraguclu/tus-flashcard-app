...
describe('ankiNoteToFields', () => {
    it('maps first two fields to Soru/Cevap and joins the rest into Kaynak', () => {
        expect(ankiNoteToFields({ fields: ['Q', 'A', 's1', 's2'], tags: [], cloze: false, hasMedia: false })).toEqual([
            'Q',
            'A',
            's1 · s2',
        ]);
    ...
    it('handles two-field notes', () => {
        expect(ankiNoteToFields({ fields: ['Q', 'A'], tags: [], cloze: false, hasMedia: false })).toEqual(['Q', 'A', '']);
    });
});
...
describe('ankiClozeToFields', () => {
    it('maps the cloze text to Text and joins the rest into Extra', () => {
        expect(
            ankiClozeToFields({ fields: ['{{c1::x}} text', 'note'], tags: [], cloze: true, hasMedia: false }),
        ).toEqual(['{{c1::x}} text', 'note']);
    });
});

describe('readAnkiNotes', () => {
    it('splits flds/tags and flags cloze (model type 1) and media', () => {
        const reader = {
            getFirstSync: () => ({ models: JSON.stringify({ '100': { type: 0 }, '200': { type: 1 } }) }),
            getAllSync: () => [
                { mid: 100, flds: 'Q1\x1fA1', tags: ' cardio ' },
                { mid: 200, flds: '{{c1::x}}\x1fextra', tags: '' },
                { mid: 100, flds: '<img src="a.png">\x1fA', tags: '' },
            ],
        };
        expect(readAnkiNotes(reader as any)).toEqual([
            { fields: ['Q1', 'A1'], tags: ['cardio'], cloze: false, hasMedia: false },
            { fields: ['{{c1::x}}', 'extra'], tags: [], cloze: true, hasMedia: false },
            { fields: ['<img src="a.png">', 'A'], tags: [], cloze: false, hasMedia: true },
        ]);
    });
});
...
describe('importAnkiReader', () => {
    const reader = {
        getFirstSync: () => ({ models: JSON.stringify({ '100': { type: 0 }, '200': { type: 1 } }) }),
        getAllSync: () => [
            { mid: 100, flds: 'Kalp?\x1fPompa\x1fFizyoloji', tags: 'exam' },
            { mid: 200, flds: '{{c1::Beyin}}\x1fnot', tags: '' },
            { mid: 100, flds: '<img src="x.png">\x1fA', tags: '' },
        ],
    };
