...
const FSRSEngine: SchedulerEngine = {
    name: 'FSRS',
    description: 'FSRS-5 (Anki 23.10+ standardÄ±)',
    ...
    schedule: (cs: CardState, grade: Grade, settings: AppSettings): ScheduleResult => {
        ...
        // ========================================
        if (cs.status === 'new' || cs.learningStep >= 0) {
            const steps = settings.learningSteps;
            const step = cs.learningStep || 0;

            const isFirstTime = !cs.stability || cs.stability === 0;

            // Zorluk: ilk kez mi yoksa gÃ¼ncellememi?
            const D = cs.difficulty > 0
                ? nextDifficulty(cs.difficulty, grade)
                : initDifficulty(grade);

            // Stability: ilk kez mi yoksa same-day mi?
            const getStability = (g: Grade) => isFirstTime ? initStability(g) : sameDayStability(cs.stability!, g);

            if (grade === 1) {
                // Again â†’ ilk adÄ±ma geri
                return {
                    interval: 0,
                    isLearning: true,
                    ...
                    stateUpdates: {
                        learningStep: 0,
                        stability: getStability(1),
                        difficulty: D,
                    },
                };
            } else if (grade === 2) {
                // Hard â†’ mevcut adÄ±m ile sÄ±radaki adÄ±mÄ±n (veya mezuniyetin) ortalamasÄ±
                let targetMinutes = 1;
                const currentMinutes = steps[step] || 1;

                if (step + 1 >= steps.length) {
                    // SÄ±radaki adÄ±m mezuniyet ise: (mevcut dk + mezuniyet dk) / 2
                    targetMinutes = Math.round((currentMinutes + (settings.graduatingInterval * 24 * 60)) / 2);
                } else {
                    // SÄ±radaki adÄ±m Ã¶ÄŸrenme adÄ±mÄ± ise: (mevcut dk + bir sonraki dk) / 2
                    const nextMinutes = steps[step + 1];
                    targetMinutes = Math.round((currentMinutes + nextMinutes) / 2);
                }

                return {
                    interval: 0,
                    isLearning: true,
                    minutesUntilDue: targetMinutes,
                    stateUpdates: {
                        learningStep: step,
                        stability: getStability(2),
                        difficulty: D,
                    },
                };
            } else if (grade === 3) {
                // Good â†’ sonraki adÄ±m veya mezuniyet
                const S = getStability(3);
                if (step + 1 >= steps.length) {
                    // Mezuniyet!
                    const I = applyFuzz(nextInterval(S, retention));
                    return {
                        interval: Math.max(settings.graduatingInterval, I),
                        isLearning: false,
                        stateUpdates: {
                            learningStep: -1,
                            stability: S,
                            difficulty: D,
                            repetition: cs.repetition + 1,
                        },
                    };
                }
                return {
                    interval: 0,
                    isLearning: true,
                    minutesUntilDue: steps[step + 1] || 10,
                    stateUpdates: {
                        learningStep: step + 1,
                        stability: S,
                        difficulty: D,
                ...
                // Easy (4) â†’ direkt mezuniyet (Ã¶ÄŸrenme aÅŸamasÄ±nda early graduation)
                const S = getStability(4);
                const I = applyFuzz(nextInterval(S, retention));
                return {
                    interval: Math.max(settings.easyInterval, I),
                    isLearning: false,
                    stateUpdates: {
                        learningStep: -1,
                        stability: S,
        ...
        // ========================================
        // PHASE 2: Review Cards (FSRS Core)
        // ========================================

        // GeÃ§en gÃ¼n hesabÄ±: dueDate'ten bugÃ¼ne kaÃ§ gÃ¼n geÃ§ti?
        const today = getToday();
        let elapsed: number;
        if (cs.dueDate && cs.interval > 0) {
            const dueDateMs = new Date(cs.dueDate).getTime();
            const todayMs = new Date(today).getTime();
            const daysSinceDue = Math.floor((todayMs - dueDateMs) / 86400000);
            elapsed = Math.max(0, cs.interval + daysSinceDue);
        } else {
            elapsed = cs.interval || 1;
        }
        if (elapsed <= 0) elapsed = 1;

        const S = cs.stability || 1;
        const D = cs.difficulty || initDifficulty(3);
        const R = forgettingCurve(elapsed, S);

        if (grade === 1) {
            // ---- LAPSE (Unutma) ----
            const newS = forgetStability(D, S, R);
            return {
                interval: 0,
                isLearning: true,
                minutesUntilDue: settings.learningSteps[0] || 1,
                stateUpdates: {
                    stability: newS,
                    difficulty: nextDifficulty(D, 1),
        ...
        // ---- RECALL (BaÅŸarÄ±lÄ± HatÄ±rlama) ----
        const newS = recallStability(D, S, R, grade);
        const newI = applyFuzz(nextInterval(newS, retention));
    ...
    previewIntervals: (cs: CardState, settings: AppSettings): IntervalPreview => {
        const retention = settings.desiredRetention || 0.9;
        const isLearning = cs.status === 'new' || cs.learningStep >= 0;
        ...
        if (isLearning) {
            ...
            const currentMinutes = steps[step] || 1;
            if (nextStep) {
                hardMinutes = Math.round((currentMinutes + nextStep) / 2);
            } else {
                hardMinutes = Math.round((currentMinutes + (settings.graduatingInterval * 24 * 60)) / 2);
            }

            return {
                again: `${steps[0]}dk`,
                hard: hardMinutes >= 1440 ? formatDays(hardMinutes / 1440) : `${hardMinutes}dk`,
                good: nextStep ? `${nextStep}dk` : `${settings.graduatingInterval} gÃ¼n`,
                easy: `${settings.easyInterval} gÃ¼n`,
            };
        }

        // Review preview â€” fuzz uygulamadan ham hesaplama
        const S = cs.stability || 1;
        const D = cs.difficulty || initDifficulty(3);

        // GerÃ§ek geÃ§en gÃ¼n: dueDate'ten bugÃ¼ne kaÃ§ gÃ¼n geÃ§ti?
        const today = getToday();
        let elapsed: number;
        if (cs.dueDate && cs.interval > 0) {
            const dueDateMs = new Date(cs.dueDate).getTime();
            const todayMs = new Date(today).getTime();
            const daysSinceDue = Math.floor((todayMs - dueDateMs) / 86400000);
            elapsed = Math.max(1, cs.interval + daysSinceDue);
        } else {
            elapsed = cs.interval || 1;
        }
        const R = forgettingCurve(elapsed, S);

        const againS = forgetStability(D, S, R);
        const hardS = recallStability(D, S, R, 2);
        const goodS = recallStability(D, S, R, 3);
        const easyS = recallStability(D, S, R, 4);

        return {
            again: `${settings.learningSteps[0]}dk`,
            hard: formatDays(nextInterval(hardS, retention)),
            good: formatDays(nextInterval(goodS, retention)),
            easy: formatDays(nextInterval(easyS, retention)),
        };
    },
};

// ============================================================
// 2. SM-2 ENGINE (Klasik Anki)
// ============================================================

const SM2Engine: SchedulerEngine = {
    name: 'SM2',
    description: 'SuperMemo-2 (Klasik Anki)',
    ...
    schedule: (cs: CardState, grade: Grade, settings: AppSettings): ScheduleResult => {
        const sm2Quality = grade === 1 ? 0 : grade === 2 ? 3 : grade === 3 ? 4 : 5;

        if (cs.learningStep !== undefined && cs.learningStep >= 0) {
            return sm2Learning(cs, sm2Quality, settings);
        }

        const { easeFactor, interval, repetition } = cs;

        if (sm2Quality === 0) {
            const newInterval = Math.max(1, Math.round(interval * settings.lapseNewInterval));
            return {
    ...
    previewIntervals: (cs: CardState, settings: AppSettings): IntervalPreview => {
        const isLearning = cs.learningStep !== undefined && cs.learningStep >= 0;
        ...
        if (isLearning) {
            ...
            return {
                again: `${steps[0]}dk`,
                hard: `${Math.round((steps[step] || 1) * 1.5)}dk`,
                good: nextStep ? `${nextStep}dk` : `${settings.graduatingInterval} gÃ¼n`,
                easy: `${settings.easyInterval} gÃ¼n`,
            };
        }

        const hardInterval = Math.max(cs.interval + 1, Math.round(cs.interval * 1.2));
        const goodInterval = Math.round(cs.interval * cs.easeFactor);
        const easyInterval = Math.round(cs.interval * cs.easeFactor * 1.3);
...
function sm2Learning(cs: CardState, quality: number, settings: AppSettings): ScheduleResult {
    const steps = settings.learningSteps;
    const step = cs.learningStep || 0;
    ...
    if (quality === 0) {
        return {
            ...
            isLearning: true,
            minutesUntilDue: steps[0] || 1,
            stateUpdates: { learningStep: 0, repetition: 0 },
        };
    }
    ...
    if (quality === 3) {
        return {
            interval: 0,
            isLearning: true,
            minutesUntilDue: Math.round((steps[step] || 1) * 1.5),
            stateUpdates: { learningStep: step },
        };
    }
    ...
    if (quality === 4) {
        ...
        return {
            ...
            isLearning: true,
            minutesUntilDue: steps[step + 1] || 10,
            stateUpdates: { learningStep: step + 1 },
        };
    }
    ...
    return {
        ...
        stateUpdates: {
            learningStep: -1,
            easeFactor: Math.max(1.3, cs.easeFactor + 0.15),
            repetition: 1,
            interval: settings.easyInterval,
        },
    };
...
// ============================================================
// 3. EXPERIMENTAL ENGINE (Anti-Gravity)
// ============================================================

const ExperimentalEngine: SchedulerEngine = {
    name: 'EXPERIMENTAL',
    description: 'Anti-Gravity (agresif tekrar)',
    ...
    schedule: (cs: CardState, grade: Grade, settings: AppSettings): ScheduleResult => {
        const currentInterval = cs.interval || 1;
        ...
        switch (grade) {
            ...
            case 2:
                newInterval = Math.max(1, Math.round(currentInterval * 0.7));
                break;
            case 3:
    ...
    previewIntervals: (cs: CardState, settings: AppSettings): IntervalPreview => {
        ...
        const interval = cs.interval || 1;
        return {
            again: `${settings.learningSteps[0]}dk`,
            hard: formatDays(Math.max(1, Math.round(interval * 0.7))),
            good: formatDays(Math.round(interval * 1.5)),
            easy: formatDays(Math.round(interval * 2.5)),
        };
    },
};
