...
| Whiteboard/scratchpad and drawing attachment | Implemented subset | geometry/photo tests, reviewer UI, and a blank drawing page that opens the full annotation editor (pen, highlighter, shapes, movable text, partial/whole eraser, undo/redo, crop, rotate) with ruled/plain paper pinned by `lib/blankCanvas.test.ts`; the saved PNG is attached as a normal `<img>` media reference | Apple Pencil-only mode is pending |
...
| CSV/TSV/TXT import/export | Implemented; update preview/confirmation and backup protect matching notes, and deck directives share the note-write transaction | delimited preview/import/export and rollback tests | Keep metadata-header fixtures |
