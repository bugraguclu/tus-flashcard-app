...
| FSRS | Implemented (FSRS-6) | `lib/fsrs.ts` pinned to upstream fsrs-rs vectors, `lib/fsrsScheduler.ts` state machine, memory state in Anki's own `cards.data` (`s`/`d`/`dr`/`decay`), history-derived states and rescheduling (`lib/fsrsMemory.ts`, `lib/fsrsMaintenance.ts`), parameter optimization (`lib/fsrsOptimizer.ts`), deck-options section, card info, `prop:s`/`prop:d`/`prop:r` search, browser sort columns, `.apkg` round trip that keeps an imported preset's retrievability review order | Add the FSRS-only review sort orders and the retention/true-retention graphs; optimizer runs on the JS thread, so very large collections are sampled |
