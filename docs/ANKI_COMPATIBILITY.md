...
| Filtered decks and custom study | Implemented | Context-preserving overview panels, two independent filters, all 11 gather search orders (0..10) stored with Anki's own ordinals (migration 10), rescheduling/preview modes with configurable delay vectors (default `[10, 60, 600, 0]`s), reviewer session countdown timers, excluded-card reporting, in-app help, deck manager, `lib/filteredDeckOptions.test.ts`, and reviewer preview queue tests | Keep parity tests aligned with upstream |
...
| Note types, conditional templates, cloze, typed answer | Implemented | AnkiMobile-aligned rich text editing toolbar (bold, italic, underline, strikethrough, remove format/Fx, subscript, superscript, color palette, ordered/unordered lists, horizontal rule, heading, font size, MathJax, raw HTML source editor), note-type template aware card generation validation (empty front allowed when reversed card generates), native or template-positioned `#typeans` input, optional focus, token-bound/bounded WebView bridge, template, bridge, settings round-trip and note manager tests | Validate more add-on filters and complex templates |
| HTML, images, audio, video, TTS | Implemented subset with an untrusted-content boundary | sanitizer/CSP, card-content, media and package tests; Anki TTS language/voice/speed parsing, installed Enhanced/Premium iOS voice preference, generation-safe single speech queue, reviewer-only focus/foreground gating and hidden-content filtering are covered by focused TTS tests; scripts, network loads, active media documents and silent navigation are blocked | Add offline MathJax and custom-font runtime QA without weakening CSP |
