...
| Typed-answer reviewer | Review one `{{type:Field}}` card with the native input and one with “Cevabı kartın içine yaz”; verify optional focus opens the keyboard, Enter reveals the answer, `#typeans` CSS applies only in-card, the typed diff is correct, and the next/repeated card starts empty. Repeat on a cloze note with `{{type:cloze:Text}}`: the input replaces the deletion in the sentence rather than following it, a card whose deletion carries a hint shows the hint when typing is off, and the answer side diffs against the deleted text |
...
| Reviewer text to speech | Install an Enhanced Turkish voice in iOS Settings; review one whole-card Turkish note and one template with `{{tts tr_TR voices=Apple_<installed_voice> speed=0.8:Field}}`; rapidly reveal/grade three cards and replay once; leave the reviewer for Settings and background/foreground once; confirm each side is spoken once, hidden hints/CSS are skipped, the requested rate is used, speech stops on blur/background and resumes only after returning to the reviewer with the prior TTS toggle preserved |
...
| Empty Cards maintenance | Open the canonical full-screen route from Decks; confirm native back gesture/header and VoiceOver labels, verify loading/empty/error states, then confirm single and bulk destructive deletion preserve the note and valid sibling cards |
...
| Photo editor capture | Annotate one photo and one blank canvas page, save both, and reopen them from the card: verify the saved PNG carries the annotations at the drawing's own resolution rather than a screen-sized copy, and that a crop and a quarter turn survive the round trip. This path has no automated coverage — a regression here is only visible on device |
| Editor toolbar peek | On the narrowest supported iPhone and one larger, open the editor and confirm the toolbar shows a partial ninth button rather than ending flush, in both orientations and with the safe-area inset applied |
| Reviewer countdown and focus | With a learn-ahead delay pending, watch the countdown tick to zero and confirm the queue rebuilds without a visible stall; open a typed-answer card and confirm the keyboard animation does not fight the card layout |
| Whiteboard across an empty queue | Draw on a card, then reach a timebox prompt, a learn-ahead countdown and the “all done” screen; confirm the ink returns when the card does, and watch for a visible flash of the canvas while no card is current |
| Permission prompts | On a fresh install, trigger the gallery, camera, microphone and notification prompts; deny one, then reopen it and confirm the app offers “Ayarları Aç” rather than a dead end |
| Tag picker | Open the tag picker on a note with a deep `::` hierarchy; confirm the enlarged search field and selection badge are comfortable to hit, indentation reads correctly to eight levels, and quick-add creates a tag under the right parent |
